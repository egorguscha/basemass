...
class Server {
    constructor() {
        ...
        this.leaderboardTime = 0; // Leaderboard update timestamp
    ...
    killPlayer(socket) {
        const { player, client } = socket;
        if (!player || !player.cells.length) return;

        while (player.cells.length) this.removeNode(player.cells[0]);

        player.cells = [];
        player.isRemoved = true;
        client.splitRequested = false;
        client.toggleSpectate = false;
        client.ejectRequested = false;
    ...
    cleanUp() {
        ...
        const date = `${now.getFullYear()}-${now.getMonth()}-${now.getDate()}`
        const time = `${now.getHours()}:${now.getMinutes()}:${now.getSeconds()}`;
        Logger.info(`Cleaned up the server ${date} ${time}`);
    }
    loadFiles() {
    ...
    startStatsServer(port) {
        // Create stats
        this.getStats({ forceUpdate: true });
        // Show stats
    ...
    getStats({ forceUpdate } = { forceUpdate: false }) {
        ...
        // Not updating leaderboard if there are no alive players or bots
        const shouldUpdateLeaderboard = (alivePlayers || bots) || forceUpdate;

        const getLeaderboard = () => {
            if (!shouldUpdateLeaderboard) {
                // Returning cached leaderboard from latest game
                return JSON.parse(this.stats).leaderboard;
            };
        ...
        var s = {
            ...
            'stats_time': Date.now(),
            'leaderboard_time': this.leaderboardTime,
