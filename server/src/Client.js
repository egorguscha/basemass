...
if (fs.existsSync(randomSkinsFile)) {
  ...
  randomSkins = fs
    .readFileSync(randomSkinsFile, "utf8")
    .split(/[\r\n]+/)
...
class Client {
  ...
  handshake_onProtocol(message) {
    ...
    this.handshakeProtocol =
      message[1] | (message[2] << 8) | (message[3] << 16) | (message[4] << 24);
    if (this.handshakeProtocol < 1 || this.handshakeProtocol > 18) {
      this.socket.close(
        1002,
        "Not supported protocol: " + this.handshakeProtocol
      );
      return;
    }
    this.handler = {
  ...
  handshake_onCompleted(protocol, key) {
    ...
    this.sendPacket(
      new Packet.SetBorder(
        this.socket.player,
        this.server.border,
        this.server.config.serverGamemode,
        "MultiOgarII " + this.server.version
      )
    );
    ...
    if (this.server.config.serverWelcome1)
      this.server.sendChatMessage(
        null,
        this.socket.player,
        this.server.config.serverWelcome1
      );
    if (this.server.config.serverWelcome2)
      this.server.sendChatMessage(
        null,
        this.socket.player,
        this.server.config.serverWelcome2
      );
    if (this.server.config.serverChat == 0)
      this.server.sendChatMessage(
        null,
        this.socket.player,
        "This server's chat is disabled."
      );
    if (this.protocol < 4)
      this.server.sendChatMessage(
        null,
  ...
  message_onSpectate(message) {
    if (message.length !== 1 || this.socket.player.cells.length !== 0) return;
  ...
  process() {
    ...
    if (this.socket.player.minionSplit) this.socket.player.minionSplit = false;
    if (this.socket.player.minionEject) this.socket.player.minionEject = false;
    this.processMouse();
  }
  getRandomSkin() {
    // Picks a random skin
  ...
  setNickname(text) {
    ...
    if (text != null && text.length > 0) {
      ...
      if (text[0] == "<" && (n = text.indexOf(">", 1)) >= 1) {
        ...
        if (n > 1) skinName = inner == "r" ? this.getRandomSkin() : inner;
        else skinName = "";
