...
class Player {
  ...
  joinGame(name, skin) {
    ...
    const alivePlayers = new Set(
      this.server.clients.flatMap((socket) => {
        const { player } = socket;
    ...
    if (!this.isMi && this.socket.isConnected != null) {
      ...
      if (client.protocol < 6) {
        client.sendPacket(
          new Packet.UpdateNodes(this, [], [], [], this.clientNodes)
        );
      }
  ...
  checkConnection() {
    ...
    if (!this.socket.isConnected) {
      ...
      if (pt && (!this.cells.length || dt >= pt)) {
        ...
        while (this.cells.length) this.server.removeNode(this.cells[0]);
      }
      this.cells = [];
      this.isRemoved = true;
      this.mouse = null;
  ...
  updateTick() {
    ...
    const posPacket = new Packet.UpdatePosition(
      this,
      this.centerPos.x,
      this.centerPos.y,
      this._scale
    ...
    const halfWidth =
      (this.server.config.serverViewBaseX + 100) / this._scale / 2;
    const halfHeight =
      (this.server.config.serverViewBaseY + 100) / this._scale / 2;
  ...
  sendUpdate() {
    ...
    while (
      ...
      if (viewNode.nodeId < clientNode.nodeId) {
        ...
        if (viewNode.isRemoved) eatNodes.push(viewNode);
          ...
          viewNode.type == 0 ||
          viewNode.type == 2 ||
          (this.server.config.serverGamemode == 3 && viewNode.type == 1)
        )
          updNodes.push(viewNode);
        ++viewIndex;
        ++clientIndex;
      }
    }
    ...
    client.sendPacket(
      new Packet.UpdateNodes(this, addNodes, updNodes, eatNodes, delNodes)
    );
    if (++this.tickLeaderboard > 25) {
      ...
      if (this.server.leaderboardType >= 0)
        client.sendPacket(
          new Packet.UpdateLeaderboard(
            this,
            this.server.leaderboard,
            this.server.leaderboardType
          )
        );
    }
  }
  updateView(len) {
    if (len) {
      ...
      if (player && !this.freeRoam) {
        ...
        if (mouseDist != 0)
          this.setCenterPos(
            this.centerPos.add(mouseVec.product(32 / mouseDist))
          );
