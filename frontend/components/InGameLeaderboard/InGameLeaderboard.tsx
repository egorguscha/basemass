...
const InGameLeaderboard = () => {
  const { top5PlayersList, currentPlayerRankData, isCurrentPlayerInTop5 } =
    useInGameLeaderboard();

  console.log(top5PlayersList);
          ...
          return (
            <li
              ...
              className={`playersList__item ${
                playersListItem.me ? "playersList__item--active" : ""
              }`}
            >
              <p className="playersList__num">{playersListItem.rank}.</p>
              <p className="playersList__name">{playersListItem.name}</p>
            </li>
          );
        })}
      </ol>
      {!isCurrentPlayerInTop5 && currentPlayerRankData && (
        <ol className="playersList__list">
          <li className="playersList__item playersList__item--active">
            <p className="playersList__num">{currentPlayerRankData.rank}</p>
            <p className="playersList__name">{currentPlayerRankData.name}</p>
          </li>
        </ol>
      )}
    </div>
