...
const SIZE_SCALE_COEF =
  Math.log(SERVER_MIN_SCALE) /
  Math.log(64 / (PLAYER_MAX_SIZE * PLAYER_MAX_CELLS));
...
(function () {
  ...
  // const USE_HTTPS = !(
  //   localStorage.getItem("SHOULD_USE_LOCAL_DEV_SERVER") === "true"
  // ); // false when using local server
  ...
  function wsMessage(data) {
    ...
    switch (packetId) {
      ...
      case 0x31: {
        ...
        const inGameleaderboardUpdateEvent = new CustomEvent(
  ...
  class Cell {
    static parseName(value) {
      ...
      let [, skin, address, domain] = value.match(
    ...
    setSkin(value) {
      ...
      const skinDeconstructionRegexp = /(.*)\|(\d+)/;
      const regExpMatches = this.skin.match(skinDeconstructionRegexp);

      const skinPlanetHash =
        regExpMatches?.[1] ||
        "ooKg2zuJu9XhZBRKQaBrEDvpeYZjDPmKREp3PMSZHLkoSFK3ejN";
      const skinCollectionId = parseInt(regExpMatches?.[2], 10) || 3808;

      const skin = new PlanetRender(skinPlanetHash, skinCollectionId);
    ...
    drawShape(ctx) {
      ...
      if (settings.showSkins && this.skin) {
        ...
        const animatedPlanetCanvas = animatedPlanet.getCurrentFrame(
          drawingSize,
          10,
          false
        );
    ...
    drawText(ctx) {
      ...
      if (this.name && settings.showNames) {
        const nameToDraw =
          this.name.length <= 12
            ? this.name
            : this.name.slice(0, 5) + ".." + this.name.slice(-5);
        drawText(
          ctx,
          false,
          this.x,
  ...
  function processKey(event) {
    let key =
      CODE_TO_KEY[event.code] || event.key ? event.key.toLowerCase() : null;
