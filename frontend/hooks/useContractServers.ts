...
import { SHOULD_USE_LOCAL_DEV_SERVER, LOCAL_DEV_SERVER } from "../constants";
...
const useContractServers = () => {
  ...
  const { data: contractServers } = useQuery({
    queryKey: ["contractServers", address],
    queryFn: () => fetchContractServers(address),
    // The query will not execute until the `` is true

    // New data on key change will be swapped without the loading state
    keepPreviousData: true,
    // Refetching only when on /dashboard page
    refetchInterval: () => (router.pathname === "/dashboard" ? 1000 : false),
  });
  ...
  useEffect(() => {
    ...
    const selectedServerUrl = contractServers[selectedServerIndex].server_url;
